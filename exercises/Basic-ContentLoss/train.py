...
def set_parameters():
    ...
    parser.add_argument('samples_size', nargs='?', type=int, default=40000)
    ...
    parser.add_argument('epoch', nargs='?', type=int, default=15)
...
def get_inputs(lr_images):
    inputs = [e for lr_image in lr_images for e in utils.crop_patches(lr_image, patch_size=args.patch_size//4)]
    return torch.stack(inputs)


def get_targets(hr_images):
    inputs = [e for hr_image in hr_images for e in utils.crop_patches(hr_image, patch_size=args.patch_size)]
    return torch.stack(inputs)


def compute_metric(outputs, targets):
    return Metric.peak_signal_to_noise_ratio(outputs, targets)
    ...
    for epoch in range(args.epoch):
        ...
        with tqdm(total=training_samples_size, ncols=100) as p_bar:
            ...
            for batch in training_dataloader:
                ...
                # 得到输入数据
                inputs = get_inputs(lr_images)

                # 得到目标数据
                targets = get_targets(hr_images)

...
from torch.cuda.amp import GradScaler, autocast # mix-precision training
...
if __name__ == '__main__':
    ...
    # 生成dataloader
    training_dataloader = DataLoader(training_dataset, batch_size=512, num_workers=num_workers, shuffle=True, pin_memory=True, drop_last=True)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=True)

    # sum = torch.zeros(size=(512, 3, 64, 64)).to('cuda:0')
    # with tqdm(total=training_samples_size, ncols=100) as p_bar:
    #     for batch in training_dataloader:
    #         # lr, hr = batch
    #         lr, gt = batch['lq'], batch['gt']
    #         lr = lr.to('cuda:0')
    #         sum += lr
    #         p_bar.update(128)
    # torch.save(sum, os.path.join(save_path, 'sum.pt'))
    # mean = torch.mean(sum, dim=0)
    # torch.save(mean, os.path.join(save_path, 'mean.pt'))
    # std = torch.std(sum, dim=0)
    # torch.save(std, os.path.join(save_path, 'std.pt'))
    ...
    checkpoint = torch.load(os.path.join(save_path, 'checkpoint-18k.pth'))
